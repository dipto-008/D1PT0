...
module.exports = {
  ...
  onStart: async function ({ message, api, event }) {
    ...
    try {
      ...
      const matchDetails = team2.find('span').text().trim().match(/\((\d+) ov, T:(\d+)\)/);
      ...
      await api.sendMessage(`❌ Error fetching the live cricket score: ${error.message}`, event.threadID, event.messageID);
    }
  }
};
