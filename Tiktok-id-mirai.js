...
module.exports.run = async function ({ api, event, args }) {
  ...
  try {
    const response = await axios.get(`https://all-image-genator-d1p.onrender.com/${ok}/tiktokid?url=${user}&num=${limit}`);
...
const options = videos.map((video, index) => `${index + 1}. ${video.title}`);
    const message = `❤️‍🩹 Choose an option Baby <💝\n` + `✿━━━━━━━━━━━━━━━━━✿\n${options.join("\n")}✿━━━━━━━━━━━━━━━━━━✿`;
    const photoUrls = [];
    const filenames = [];
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
  ...
  if (event.type == "message_reply") {
    ...
    if (isNaN(reply) || reply < 1 || reply > handleReply.options.length) {
      return api.sendMessage(`Please reply with a number between 1 and ${handleReply.options.length}.`, event.threadID, event.messageID);
    }
    try {
...
handleReply.filenames.forEach(filename => {fs.unlinkSync(filename);});}, event.messageID);
    } catch (error) {
